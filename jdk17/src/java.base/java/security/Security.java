...
public final class Security {
    ...
    private static void initialize() {
        ...
        if ("true".equalsIgnoreCase(props.getProperty
            ...
            // <java.home>/conf/security/openjsse.security
            // org.openjsse platform module
            boolean openJSSEEnabled = "true".equals(VM.getSavedProperty("org.openjsse.provider"));

            if (openJSSEEnabled) {
                File openjssePropFile = securityPropFile("openjsse.security");
                Optional<java.lang.module.ModuleReference> omref = java.lang.module.ModuleFinder.ofSystem().find("org.openjsse");
                if (openjssePropFile.exists() && omref.isPresent()) {
                    try (FileInputStream fis = new FileInputStream(openjssePropFile);
                        ...
                        if(handlerPkgs == null || handlerPkgs.isEmpty()) {
                            System.setProperty("java.protocol.handler.pkgs",
                                               "org.openjsse.net.www.protocol");
                        } else if (!handlerPkgs.contains("org.openjsse.net.www.protocol")){
                            System.setProperty("java.protocol.handler.pkgs",
                                    "org.openjsse.net.www.protocol|"+handlerPkgs);
                        }
                        if (sdebug != null) {
                            sdebug.println("reading openjsse properties file: " +
                                        propFile);
                        }
                    } catch (IOException e) {
                        throw new ProviderException(
                                "Unable to load OpenJSSE provider properties");
                    }
                } else {
                    throw new ProviderException(
                            "OpenJSSE provider requested but not found");
                }
            }
        }
